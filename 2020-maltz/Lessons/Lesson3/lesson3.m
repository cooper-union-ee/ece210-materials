...
x([1 3 5 7]);            % Returns a vector with the 1st, 3rd, 5th, and 7th entries of x
x(1:2:7);                % Same thing uses the colon operator to create the index vector
x([2:4 3:6]);
x([1:4]) = 7;            % Changes the values from the 1st and 4th entries of x to 7
...
A(1:3,1:2);              % Returns the entries in the first three rows AND first two columns
A(end,:);
...
A(A > 12)               % returns entries of A which are greater than 12 in a column vector

%%
% Let's see how MATLAB does this

A > 12                  % returns a logical array with entries corresponding to the entries of A
...
% inline function. Writing functions is very important in MATLAB. Namely,
